...
taskList = do
  ...
  rec let todos' = todos todos0 $ leftmost
      ...
      (m, (e, _)) <- splitV (pure (subtract 6)) (pure (True, True)) todos' $
        ...
        (pure (subtract 3))
        (pure (True, True))
        btn (display $ Map.size <$> current m)
  return ()
...
todo t0 = do
  ...
  rec let checkboxWidth = 3
      ...
      (ti, d) <- pane labelRegion (pure True) $ do
        ...
        let deleteSelf =
              attachWithMaybe backspaceOnEmpty (current $ _textInput_value v) i
        return (v, deleteSelf)
  return $ TodoOutput
...
scrolling = col $ do
  ...
  fixed 1 $ text $ ffor out $ \(ix, total) ->
    "Scrolled to line " <>
    T.pack (show ix) <>
    " of " <>
    T.pack (show total)
  ...
  rec tabNav <- tabNavigation
      ...
      let delete = ffor todoDelete $ \k -> Map.singleton k Nothing
          ...
          todoDelete = switch . current $
            leftmost .
            Map.elems .
            Map.mapWithKey (\k -> (k <$) . _todoOutput_delete) <$>
            listOut
          todosMap = joinDynThroughMap $ fmap _todoOutput_todo <$> listOut
          insert = ffor (tag (current todosMap) newTodo) $
                   \m -> case Map.lookupMax m of
          ...
          selectOnDelete = fanMap $ (`Map.singleton` ()) <$> attachWithMaybe
            (\m k -> let (before, after) = Map.split k m in
                fmap fst $ Map.lookupMax before <|> Map.lookupMin after)
            (current todosMap)
            todoDelete
  return listOut
